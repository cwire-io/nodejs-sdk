...
class CWireWebSocket {
    ...
    initListeners() {
        ...
        this.socket.on('connect', () => {
            ...
            this.cwire
                .getLogger()
                .info(CONNECT_TO_CWIRE_LOGGER_PREFIX, `Connected to cwire api as ${((_a = this.cwire.getWorker()) === null || _a === void 0 ? void 0 : _a.name) || 'unknown'} successfully`);
        });
        this.socket.on('disconnect', (error) => {
            this.cwire
                .getLogger()
                .error(DISCONNECT_TO_CWIRE_LOGGER_PREFIX, `Disconnected to cwire api with the error ${error}`);
        });
        this.socket.on('error', (error) => {
            this.cwire
                .getLogger()
                .error(CONNECTION_ERROR_LOGGER_PREFIX, `Connection error ${error.toString()}`);
        });
