...
class CWireWebSocket {
    constructor(cwire) {
        this.onWorkerFunctionCalled = (functionName, params, resolve) => __awaiter(this, void 0, void 0, function* () {
            ...
            this.cwire
                .getLogger()
                .system(logger_1.FUNCTION_CALL_PERFORMANCE_LOGGER_PREFIX, `${functionName}: ${perf_hooks_1.performance.now() - t0}ms`);
        });
    ...
    initListeners() {
        ...
        this.socket.on('connect', () => {
            ...
            this.cwire
                .getLogger()
                .info(logger_1.CONNECT_TO_CWIRE_LOGGER_PREFIX, `Connected to cwire api as ${((_a = this.cwire.getWorker()) === null || _a === void 0 ? void 0 : _a.name) || 'unknown'} successfully`);
        });
        this.socket.on('disconnect', (error) => {
            this.cwire
                .getLogger()
                .error(logger_1.DISCONNECT_TO_CWIRE_LOGGER_PREFIX, `Disconnected to cwire api with the error ${error}`);
        });
        this.socket.on('error', (error) => {
            this.cwire
                .getLogger()
                .error(logger_1.CONNECTION_ERROR_LOGGER_PREFIX, `Connection error ${error.toString()}`);
        });
