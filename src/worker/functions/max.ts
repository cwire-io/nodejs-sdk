import { MAX_LOGGER_PREFIX } from '../../constants/logger';
  ...
  async controller(
    ...
    try {
      ...
      const data = await dataModel.calculate(
        this.cwire,
        DataModelCalculationFunctions.MAX,
        fieldName,
        query,
      );
      Logger.system(MAX_LOGGER_PREFIX, `Run max calculation`);
      return { success: true, data };
    } catch (error) {
      Logger.error(
        MAX_LOGGER_PREFIX,
        `Error on max calculation: ${error.toString()}`,
      );
